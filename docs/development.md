...
Selectable weapon cosmetics use these Garry's Mod Workshop addons:
...
`resource.AddWorkshop` in `gamemode/init.lua` makes clients download individual items; it does not install them on the dedicated server. Put the same items in the collection passed through `+host_workshop_collection <collectionID>` so server-side model validation can enable them. The picker always lists the configured catalog; models missing on the server are rejected and the stock knife/magnum remain available.
